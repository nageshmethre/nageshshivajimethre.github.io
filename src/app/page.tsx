    ...
    <div className="min-h-screen bg-[#030712] text-[#f3f4f6] flex flex-col justify-between selection:bg-purple-500/30 selection:text-purple-300">
      ...
      {activeProjectModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg glass-panel border border-white/10 rounded-2xl shadow-2xl p-6 relative space-y-4">
            <div className="flex items-center justify-between border-b border-white/5 pb-2">
              <h3 className="text-lg font-bold text-white">{activeProjectModal.title}</h3>
              <span className="text-xs font-bold text-purple-400 uppercase bg-purple-500/10 px-2 py-0.5 rounded">
                {activeProjectModal.category}
              </span>
            </div>
            
            <p className="text-xs text-gray-300 leading-relaxed">{activeProjectModal.longDescription}</p>

            <div className="flex flex-wrap gap-1.5">
              {activeProjectModal.tags.map((tag, idx) => (
                <span key={idx} className="text-[10px] font-semibold bg-white/5 text-gray-400 px-2 py-0.5 rounded">
                  {tag}
                </span>
              ))}
            </div>
            ...
            <div className="flex gap-3 pt-4 justify-end border-t border-white/5">
              ...
              <a 
                ...
                target="_blank"
                className="px-4 py-2 bg-white/5 hover:bg-white/10 text-white text-xs font-bold uppercase tracking-wider rounded-xl border border-white/5 flex items-center gap-1.5 transition-all"
              >
              ...
              {activeProjectModal.live && (
                <a 
                  ...
                  target="_blank"
                  className="px-4 py-2 bg-gradient-to-tr from-purple-500 to-cyan-400 text-white text-xs font-bold uppercase tracking-wider rounded-xl flex items-center gap-1.5 transition-all shadow"
                >
